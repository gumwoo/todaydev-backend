...
- stream token은 Redis에서 검증 즉시 소비되는 1회용 토큰이다.
- 브리핑 생성 요청은 `GENERATING` 상태를 즉시 반환하고 비동기 작업이 진행된다.
- 진행 이벤트는 Redis buffer에 저장되어 연결 시점이 늦어도 재생할 수 있다.
