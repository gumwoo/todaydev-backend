...
- API 구현 전 반드시 `API_CONTRACT.md`를 먼저 확인한다.
- 응답 포맷, 에러 포맷, enum, SSE event name은 `API_CONTRACT.md`를 따른다.
- 계약과 구현이 다르면 코드를 임의로 맞추지 말고 계약 변경 여부를 먼저 판단한다.
- 백엔드에서 계약을 바꾸면 프론트 `API_CONTRACT.md`도 같은 내용으로 갱신한다.
- Controller, DTO, ErrorCode, SSE event 구현은 계약 문서와 이름이 일치해야 한다.
